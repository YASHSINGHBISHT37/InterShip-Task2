        ...
        <div className="flex flex-col gap-3">
            ...
            {!alertDismissed && (
                ...
                >
                    ...
                    <p className="text-[10px] text-yellow-300 leading-6">6 members expiring in 7 days — tap to send WhatsApp reminders</p>
                </button>
            )}
...
export function MemberListScreen() {
    const allMembers = [
        { name: 'Shivam Sharma', phone: '98765 43210', status: 'Active', plan: 'Gold', sc: 'bg-green-500/20 text-green-400' },
        { name: 'Pratham Sharma', phone: '88220 11122', status: 'Active', plan: 'Silver', sc: 'bg-yellow-500/20 text-yellow-400' },
            ...
            <div className="rounded-xl overflow-hidden border border-white/10">
                ...
                {filtered.map((m, i) => (
                    <div
                        ...
                        className={`grid grid-cols-4 px-3 py-2.5 gap-2 items-center cursor-pointer duration-250 hover:bg-white/5 transition-colors ${i !== filtered.length - 1 ? 'border-b border-white/[0.06]' : ''} ${selected === m.name ? 'bg-indigo-500/10' : ''}`}
                    >
                        <span className="text-xs text-white/80 font-medium">{m.name}</span>
                        <span className="text-[10px] text-white/40">{m.phone}</span>
                        <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full w-fit ${m.sc}`}>{m.status}</span>
