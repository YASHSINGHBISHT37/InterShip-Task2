    ...
    return (
        <div className="flex flex-col gap-3">
            <div className="flex rounded-xl overflow-hidden border border-white/10">
                {['Owner', 'Mechanic'].map((r) => (
                    <button key={r} onClick={() => setRole(r)}
                        className={`flex-1 cursor-pointer transition-all duration-300 ease-in-out py-2 text-xs font-medium ${r === role ? 'text-white' : 'bg-transparent text-white/40 hover:text-white/70'}`}
                        style={r === role ? { backgroundColor: '#5F3239' } : {}}>
                        {r}
                    </button>
                ))}
            </div>
            <div>
                ...
                <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="e.g. workshop_admin"
                    className="w-full bg-white/5 border border-white/10 rounded-full px-4 py-2 text-xs text-white placeholder-white/30 outline-none focus:border-[#5F3239]" />
            </div>
            <div>
                <div className="flex justify-between mb-1">
                    <label className="text-[9px] uppercase tracking-widest text-white/30">PIN / Password</label>
                    <span onClick={() => setShowPin(!showPin)} className="text-[9px] cursor-pointer hover:opacity-70" style={{ color: '#c47a82' }}>
                        {showPin ? 'HIDE' : 'SHOW'}
                    </span>
                </div>
                <input type={showPin ? 'text' : 'password'} value={pin} onChange={(e) => setPin(e.target.value)} placeholder="••••"
                    className="w-full bg-white/5 border border-white/10 rounded-full px-4 py-2 text-xs text-white placeholder-white/40 tracking-[0.3em] outline-none focus:border-[#5F3239]" />
            </div>
            <div className="flex items-center justify-between">
                <label className="flex items-center gap-1.5 text-[10px] text-white/40 cursor-pointer">
                    <span onClick={() => setRemember(!remember)}
                        className={`w-3 h-3 rounded border flex items-center justify-center ${remember ? 'border-[#5F3239]' : 'border-white/20'}`}
                        style={remember ? { backgroundColor: '#5F3239' } : {}}>
                        {remember && <span className="text-[8px] text-white">✓</span>}
                    </span>
                    Remember me
                </label>
                <span className="text-[10px] cursor-pointer hover:opacity-70" style={{ color: '#c47a82' }}>Forgot PIN?</span>
            </div>
            <button onClick={handleLogin}
                className="w-full py-2.5 cursor-pointer rounded-lg text-xs font-semibold tracking-widest transition-colors text-white"
                style={{ backgroundColor: '#5F3239' }}
                onMouseEnter={e => e.target.style.backgroundColor = '#7a3f47'}
                onMouseLeave={e => e.target.style.backgroundColor = '#5F3239'}>
                LOGIN →
            </button>
            {status && <p className="text-center text-[10px]" style={{ color: '#d4959b' }}>{status}</p>}
            <p className="text-center text-[10px] text-white/30">
                New here? <span className="cursor-pointer hover:opacity-70" style={{ color: '#c47a82' }}>Contact admin for access</span>
            </p>
        </div>
    )
}
...
export function DashboardScreen() {
    ...
    const jobs = [
        { vehicle: 'UP78 AB 4521', status: 'New', color: 'bg-[#5F3239]' },
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-1.5">
                {['All', 'New', 'In Progress', 'Ready'].map((f) => (
                    <button key={f} onClick={() => setFilter(f)}
                        className={`px-2.5 py-1 border border-white/10 cursor-pointer rounded-lg text-[10px] font-medium transition-colors ${f === filter ? 'text-white' : 'text-white/40 hover:text-white/70'}`}
                        style={f === filter ? { backgroundColor: '#5F3239' } : {}}>
                        {f}
                    </button>
                ))}
            </div>
            ...
            <button onClick={() => alert('Opening new job card form...')}
                className="w-full py-2.5 cursor-pointer rounded-lg text-xs font-semibold tracking-widest transition-colors text-white"
                style={{ backgroundColor: '#5F3239' }}
                onMouseEnter={e => e.target.style.backgroundColor = '#7a3f47'}
                onMouseLeave={e => e.target.style.backgroundColor = '#5F3239'}>
    ...
    return (
        <div className="flex flex-col gap-3">
            <div>
                ...
                <input value={regNo} onChange={(e) => handleRegLookup(e.target.value)} placeholder="UP78 AB 1234"
                    className="w-full bg-white/5 border border-white/10 rounded-full px-4 py-2 text-xs text-white placeholder-white/30 outline-none focus:border-[#5F3239] font-mono" />
            </div>
            {customer && (
                <div className="rounded-lg px-3 py-2 text-[10px]" style={{ backgroundColor: '#5F323915', border: '1px solid #5F323940', color: '#d4959b' }}>
            ...
            <div>
                ...
                <textarea value={complaint} onChange={(e) => setComplaint(e.target.value)}
                    placeholder="e.g. Engine noise, oil change due 🎤" rows={3}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-white placeholder-white/30 outline-none focus:border-[#5F3239] resize-none" />
            </div>
            <div>
                ...
                <select value={mechanic} onChange={(e) => setMechanic(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 rounded-full px-3 py-2 text-xs text-white outline-none focus:border-[#5F3239]">
            ...
            <div className="grid grid-cols-2 gap-2">
                <button onClick={() => { setRegNo(''); setCustomer(''); setComplaint('') }}
                ...
                <button onClick={() => alert('Job card created!')}
                    className="py-2 rounded-xl cursor-pointer text-[11px] font-semibold text-white transition-colors"
                    style={{ backgroundColor: '#5F3239' }}
                    onMouseEnter={e => e.target.style.backgroundColor = '#7a3f47'}
                    onMouseLeave={e => e.target.style.backgroundColor = '#5F3239'}>
                    CREATE JOB
    ...
    return (
        <div className="flex flex-col gap-3">
            <input value={query} onChange={(e) => setQuery(e.target.value)}
                ...
                className="w-full bg-white/5 border border-white/10 rounded-full px-3 py-2 text-[10px] text-white placeholder-white/30 outline-none focus:border-[#5F3239]" />
            <div className="flex flex-col gap-2">
                {filtered.length === 0 && <div className="px-3 py-3 text-[10px] text-white/30">No records match "{query}"</div>}
                {filtered.map((r, i) => (
                    <div key={i} onClick={() => setExpanded(expanded === i ? null : i)}
                        ...
                        {expanded === i && (
                            <div className="mt-2 pt-2 border-t border-white/10 flex flex-col gap-1">
                                ...
                                <p className="text-[10px] font-semibold" style={{ color: '#d4959b' }}>Total: {r.total}</p>
                            </div>
                        )}
                    </div>
                ))}
    ...
    return (
        <div className="flex flex-col gap-3">
            <div className="flex gap-2">
                <input placeholder="Search part name"
                    className="flex-1 bg-white/5 border border-white/10 rounded-full px-3 py-2 text-[10px] text-white placeholder-white/30 outline-none focus:border-[#5F3239]" />
                <button onClick={() => alert('Open add-part form')}
                    className="px-4 cursor-pointer py-1.5 rounded-full text-[10px] font-semibold text-white transition-colors"
                    style={{ backgroundColor: '#5F3239' }}
                    onMouseEnter={e => e.target.style.backgroundColor = '#7a3f47'}
                    onMouseLeave={e => e.target.style.backgroundColor = '#5F3239'}>
                    + PART
            ...
            {lowStock.length > 0 && (
                <div className="rounded-full px-3 py-2 text-[10px]" style={{ backgroundColor: '#5F323915', border: '1px solid #5F323950', color: '#d4959b' }}>
            ...
            <div className="rounded-xl overflow-hidden border border-white/10">
                ...
                {parts.map((p, i) => (
                    <div key={i}
                        className={`grid grid-cols-4 px-3 py-2.5 items-center ${i !== parts.length - 1 ? 'border-b border-white/[0.06]' : ''}`}
                        style={p.stock < p.reorder ? { backgroundColor: '#5F323908' } : {}}>
                        <span className="text-xs text-white/80 font-medium">{p.name}</span>
                        <span className="text-[10px] font-semibold" style={p.stock < p.reorder ? { color: '#c47a82' } : { color: 'rgba(255,255,255,0.5)' }}>{p.stock}</span>
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-3 gap-2">
                <button onClick={() => setPaid(true)}
                    className="py-2 rounded-xl cursor-pointer text-[10px] font-semibold text-white transition-colors"
                    style={{ backgroundColor: '#5F3239' }}
                    onMouseEnter={e => e.target.style.backgroundColor = '#7a3f47'}
                    onMouseLeave={e => e.target.style.backgroundColor = '#5F3239'}>
                    {paid ? 'PAID ✓' : 'MARK PAID'}
                </button>
                <button onClick={() => alert('Sending bill via WhatsApp...')}
                ...
                <button onClick={() => alert('Printing bill...')}
                    className="py-2 rounded-xl border border-white/15 cursor-pointer text-[10px] text-white/50 hover:border-white/30 transition-colors">
                    PRINT
                </button>
            </div>
